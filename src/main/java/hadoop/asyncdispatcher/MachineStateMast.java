 ...
 *
 */
public class MachineStateMast extends CompositeService {
	private Dispatcher dispatcher; // 中央异步调度器
	private String jobId;
	private int taskNumber; // 改作业包含的任务数目
	private String[] taskIDs; // 改作业包含的所有任务

	public MachineStateMast(String name, String jobID, int taskNumber) {
		super(name);
		this.jobId = jobID;
	...
	public void serviceInit(final Configuration conf) throws Exception {
		...
		dispatcher.register(JobEventType.class, new JobStateMachine(dispatcher.getEventHandler()));
		//分别注册job和task事件调度器
