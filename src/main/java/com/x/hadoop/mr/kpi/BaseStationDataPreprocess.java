...
public class BaseStationDataPreprocess extends Configured implements Tool 
{
	 ...
	 * 在不同CGI停留的时长
	 */ 
	public static class Reduce extends Reducer<Text, Text, NullWritable, BaseStationInfo> 
	{
		private String date;
		...
		public void reduce ( Text key, Iterable<Text> values, Context context ) throws IOException, InterruptedException
		{
			...
			try 
			{
				...
				{   baseStationInfo.clear();
				    baseStationInfo.setImsi(imsi);
				    baseStationInfo.setCgi(entry.getKey());
				    baseStationInfo.setTimeFlag(timeFlag);
				    baseStationInfo.setStayTime(entry.getValue());
					
					context.write( NullWritable.get(), baseStationInfo );
				}
			}
			catch ( Exception e )
			{
	...
	public int run(String[] args) throws Exception {
		...
		job.setOutputValueClass( BaseStationInfo.class );
